        ...
        if m:
            ...
            if imgStart.startswith('File'):
                ...
                try:
                    ...
                    if runto=='local':
                        newImg += imgd['name']    # para debug, añado el nombre de la imagen
                        
                    #ancho / alto > 1 : horizontal
                    #ancho / alto < 1 : vertical
                        
                    ratio = im.size[0] / im.size[1]
                    
                    self.debug ("image format:" + im.format + " mode:"+ im.mode+ "image size:{ancho}x{alto} ratio:{rat}".format(ancho=im.size[0], alto=im.size[1], rat=ratio))
                ...
                #title
                alttext=img[1].split('=')[1]
                lnk[img[1].split('=')[0]] = alttext.replace('"', '\'')
...
class myRevista(object):
    ...
    def go(self):
        ...
        filelines.append(bytes('<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">', 'UTF-8'))
        filelines.append(bytes('<HTML><HEAD><meta http-equiv="Content-Type" content="text/html; charset=utf-8">', 'UTF-8'))
        filelines.append(bytes('<TITLE>SERRABLO '+ self.numrev +'</TITLE>', 'UTF-8'))
        filelines.append(bytes('</HEAD><BODY>', 'UTF-8'))
        ...
        filelines.append(bytes('</BODY></HTML>', 'UTF-8'))   
        ...
        for linea in filelines:
            ...
            #print(str(linea, encoding='utf8'))
        #print(allin) 
        
        regex = re.compile(r'<li>.*?</li>', re.M|re.I)  
        mach = regex.search(allin)
        if mach:
            allout = regex.sub( '* * * * * *', allin, 1)  
